...
contract("Betting", function (accounts) {
  ...
  describe("Oracle", async () => {
    it("New token balance", async () => {
      const tokBala = web3.utils.fromWei(
  ...
  describe("TokenTransfer", async () => {
    ...
    it("transfer tokens to acct1", async () => {
      await token.transfer(accounts[1], "250", { from: accounts[0] });
      await token.transfer(accounts[2], "150", { from: accounts[0] });
      await token.approve(oracle.address, "400", { from: accounts[0] });
      await token.approve(oracle.address, "250", { from: accounts[1] });
      await token.approve(oracle.address, "150", { from: accounts[2] });
      await oracle.depositTokens("400", { from: accounts[0] });
      await oracle.depositTokens("250", { from: accounts[1] });
      await oracle.depositTokens("150", { from: accounts[2] });
    });

    it("New token balance", async () => {
      const tokBal10 = web3.utils.fromWei(
        (await oracle.adminStruct(accounts[0])).tokens,
        "wei"
      );
      const tokBal11 = web3.utils.fromWei(
        (await oracle.adminStruct(accounts[1])).tokens,
        "wei"
      );
      const tokBal12 = web3.utils.fromWei(
        (await oracle.adminStruct(accounts[2])).tokens,
        "wei"
      );
      const tokBal13 = web3.utils.fromWei(
  ...
  describe("setupBets", async () => {
    ...
    it("See updated odds, should be new 2000", async () => {
      ...
      const bn = new web3bn.BN(result2);
    ...
    it("New token balance", async () => {
      const tokBal1 = web3.utils.fromWei(
        (await oracle.adminStruct(accounts[0])).tokens,
        "wei"
      );
    ...
    it("should be old 2000, not new 3000", async () => {
      ...
      const bn = new web3bn.BN(result3);
      //const str = bn.toString(16);
      const str = bn.toString(16).padStart(64, "0");
      const pieces = str
        .match(/.{1,2}/g)
