...
contract Betting {
        ...
        returns (uint8, uint256)
    {
        ...
        for (matchSlot = 0; matchSlot < 32; matchSlot++) {
            ...
            // if 0 or 1, this represents the home or away as winner, respectively
            if (winningTeam != 2) {
        ...
        // payout 95% due to oracle payment
    ...
    function fundBook(bool iWantTokens) external payable {
        ...
        if (iWantTokens && contractTokens > 0) {
            token.transfer(msg.sender, netinvestment);
            lpStruct[msg.sender].outEpoch = betEpoch + 8;
            emit Cashflow(msg.sender, netinvestment, "fund tokens");
        }
    ...
    // the away odds calculated using this function
