...
const web3a = new Web3(connectionString);
...
contract("Betting", function (accounts) {
  ...
  describe("setupBets", async () => {
    it("checkHour", async () => {
      _timestamp = (await web3a.eth.getBlock(await web3a.eth.getBlockNumber()))
        .timestamp;
    ...
    it("fast forward 6 hours", async () => {
      ...
      const bnb = new web3.BN(betData7b);
      //const str = bn.toString(16);
      const strb = bnb.toString(16).padStart(64, "0");
      const piecesb = strb
        .match(/.{1,2}/g)
        .reverse()
        .join("")
